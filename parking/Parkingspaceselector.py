...
img = cv2.imread("parking\parking\screenshot.png")
...
def mouseClick(events, x, y, flags,args):
    ...
    with open('parking\car_positions','wb') as f: # used to save the rectangle to a file
        pickle.dump(posList,f)


while True:
    img=cv2.imread("parking\screenshot.png") # we are reloading the image everytime so that we are able to delete rectangle with right clicks
    for pos in posList:
